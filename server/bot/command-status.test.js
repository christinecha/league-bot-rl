...
const expectQueueMessage = (msg) => expect.objectContaining({
  fields: [
...
test("@LeagueBot status <teamSize>", async (done) => {
  ...
  expect(send).toHaveBeenNthCalledWith(1, expectQueueMessage(queueToString(queue1)));
  expect(send).toHaveBeenNthCalledWith(2, expectQueueMessage(queueToString(queue2)));

  // When the queue is empty, it should say that instead.
  await discord.trigger("message", msg(user1, `<@!${BOT_ID}> status 3s`));
  expect(send).toHaveBeenNthCalledWith(3, expectQueueMessage('No one in the queue.'));
